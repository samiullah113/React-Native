...
const ColorModel=({navigation})=>{
    ...
    const handleSubmit=useCallback(()=>{
        if(!name){
            alert('please enter Palete Name');
        }
         else if(selectedColors.length<3){
            alert('Please Select Atleast three colors');
         }
        else{
            const newColor={
                paletteName:name,
    ...
    const handleChange=useCallback((value,color)=>{
        ...
        setSelectedColors(colors=>[...colors,color])
            ...
            setSelectedColors(colors=>colors.filter(selectedColors=>color.colorName!==selectedColors.colorName),);
        }
    },[])
    ...
    return(
        <View style={{flex:1}}>
            ...
            <Text style={{marginHorizontal:10,fontSize:16,fontWeight:'bold' }}>
                Enter The Name Of the Palette:
            </Text>
            <TextInput value={name} onChangeText={setName} placeholder='Eneter name' style={[styles.input]} />
            ...
            <TouchableOpacity onPress={handleSubmit}>
                ...
                <Text style={[styles.btn ,{textAlign:'center',position:'relative'}]}> submit</Text>
            </TouchableOpacity>
        </View>
    )
}
const styles = StyleSheet.create({
     color:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',padding:10,borderBottomColor:'black',borderBottomWidth:2},
    input: {
